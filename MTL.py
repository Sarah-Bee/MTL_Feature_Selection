...
def loss (X, y, W):
    N = X.shape[1]
    y_predict = [np.sum( X[:, i, :] * W, axis = 1) for i in range(N)]
    y_predict = activationSig(np.array(y_predict)).T
    epsilon = 1e-10
    return -1 / (N * t ) * np.sum (y * np.log(y_predict + epsilon) + (1 - y) * np.log(1 - y_predict) ) 

def lNorm (W, t, m):
    return np.sum( [np.sqrt (np.sum(np.fromiter( (W[i, j] ** 2 for i in range(t)) , float)) ) for j in range(m)] )
...
class analysis:
    def __init__( self, X, y, W, limit):
        ...
        N = self.X.shape[1]
        y_predict = [np.sum( self.X[:, i, :] * self.W, axis = 1) for i in range(N)]
...
for k, window in windows.split(range(m)):
    ...
    X_train = [ windowData[i][ :, :-1]   for i in taskList ]                    
    X_train = np.array(X_train)
    y_train = [ windowData[i][ :, -1]   for i in taskList ]                    
    y_train = np.array(y_train)

    X_test = [ windowData_test[i][ :, :-1]   for i in taskList ]  
    X_test = np.array(X_test)
    y_test = [ windowData_test[i][ :, -1]   for i in taskList ]
    y_test = np.array(y_test)
    
    # Initialise Parameters
    # Weight matrix
    W = np.zeros([t, windowDataRange.shape[0] - 1 ])
    ...
    while reg_param > 0: # Should be 0
        ...
        # Regularisation loss
        T_x = reg_param * lNorm(W_x, t, m_window)
    ...
    results = [Window, e, c, Z, f1, error, W]
    Results.append(results)

    np.save('MTL_Boosted_Sensitivity_Results_Final_5.npy', Results)
